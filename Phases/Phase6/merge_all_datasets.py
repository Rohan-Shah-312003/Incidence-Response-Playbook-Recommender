...
# Input paths - all possible data sources
CERT_PATH = "./data/cert_processed/cert_incidents.csv"
ENRON_PATH = "./data/enron_processed/enron_incidents.csv"
VERIS_PATH = "./data/veris_processed/veris_incidents.csv"
DDOS_MULTI_PATH = "./data/ddos_incidents/ddos_incidents_multi_source.csv"
DDOS_CLOUDFLARE_PATH = "./data/cloudflare_ddos/cloudflare_ddos_incidents.csv"

# Output path
OUTPUT_PATH = "./data/real_incidents_balanced_v2.csv"
...
def balance_classes(df, target_per_class=TARGET_PER_CLASS):
    ...
    for label in sorted(df['incident_type'].unique()):
        ...
        if current_count > target_per_class:
            # Downsample
            print(f"{label:20s}: {current_count:4d} → {target_per_class:4d} (downsampling)")
            subset = resample(
                subset, 
            ...
            print(f"{label:20s}: {current_count:4d} → {target_per_class:4d} (upsampling)")
            subset = resample(
                subset, 
                n_samples=target_per_class, 
                random_state=42,
